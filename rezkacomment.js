(function () {
  ...
  async function getEnTitle(id, type) {
    try {
      ...
      let enTitle = enTranslation?.data?.title || enTranslation?.data?.name;

      // The /translations sub-resource often contains an en-US entry whose
      // title/name field is empty (nobody submitted a manual English
      // translation on TMDB). Fall back to a direct request with
      // language=en-US: TMDB resolves this server-side and returns the
      // original title when no dedicated translation exists.
      if (!enTitle) {
        console.log('[RezkaComment] no title in translations cache, falling back to language=en-US request for', tmdbCacheKey);
        ...
        enTitle = fallback.title || fallback.name;
      }
